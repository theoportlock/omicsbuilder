...
def describe(subject, pval=0.05, corr=None, change=None):
    # CHANGED
    ...
    if change:
        ...
        increased = 'sig increased = ' +\
            str(df.loc[(df['sig'].lt(pval)) & (df[df.columns[df.columns.str.contains('Log2')]].gt(0).iloc[:,0]), 'sig'].lt(pval).sum()) +\
            '/' +\
            str(df.shape[0]) +\
            ' (' +\
            str(round(df.loc[(df['sig'].lt(pval)) & (df[df.columns[df.columns.str.contains('Log2')]].gt(0).iloc[:,0]), 'sig'].lt(pval).sum()/df.shape[0] * 100)) +\
            '%)'
        # DECREASED
        decreased = 'sig decreased = ' +\
            str(df.loc[(df['sig'].lt(pval)) & (df[df.columns[df.columns.str.contains('Log2')]].lt(0).iloc[:,0]), 'sig'].lt(pval).sum()) +\
            '/' +\
            str(df.shape[0]) +\
            ' (' +\
            str(round(df.loc[(df['sig'].lt(pval)) & (df[df.columns[df.columns.str.contains('Log2')]].lt(0).iloc[:,0]), 'sig'].lt(pval).sum()/df.shape[0] * 100)) +\
            '%)'
